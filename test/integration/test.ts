...
test('husky6 Jira ticket ID should be in commit message', async (t: ExecutionContext) => {
  await testCommitMessage(singleScopeMessage, 'husky6', t);
  await exec('npm run cleanup:husky:6 && npm run prepare:husky:6', './', t);
  await testCommitMessage(hyphenatedScopeMessage, 'husky6', t);
  await exec('npm run cleanup:husky:6 && npm run prepare:husky:6', './', t);
  await testCommitMessage(firstLineWithCommentMessage, 'husky6', t);
  await exec('npm run cleanup:husky:6 && npm run prepare:husky:6', './', t);
  await testCommitMessage(imitateVerboseCommit, 'husky6', t);
});



test('husky7 Jira ticket ID should be in commit message', async (t: ExecutionContext) => {
  await testCommitMessage(singleScopeMessage, 'husky7', t);
  await exec('npm run cleanup:husky:7 && npm run prepare:husky:7', './', t);
  await testCommitMessage(hyphenatedScopeMessage, 'husky7', t);
  await exec('npm run cleanup:husky:7 && npm run prepare:husky:7', './', t);
  await testCommitMessage(firstLineWithCommentMessage, 'husky7', t);
  await exec('npm run cleanup:husky:7 && npm run prepare:husky:7', './', t);
  await testCommitMessage(imitateVerboseCommit, 'husky7', t);
});



test('husky8 Jira ticket ID should be in commit message', async (t: ExecutionContext) => {
  await testCommitMessage(singleScopeMessage, 'husky8', t);
  await exec('npm run cleanup:husky:8 && npm run prepare:husky:8', './', t);
  await testCommitMessage(hyphenatedScopeMessage, 'husky8', t);
  await exec('npm run cleanup:husky:8 && npm run prepare:husky:8', './', t);
  await testCommitMessage(firstLineWithCommentMessage, 'husky8', t);
  await exec('npm run cleanup:husky:8 && npm run prepare:husky:8', './', t);
  await testCommitMessage(imitateVerboseCommit, 'husky8', t);
});
